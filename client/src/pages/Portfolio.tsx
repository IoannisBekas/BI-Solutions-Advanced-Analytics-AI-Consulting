...
function ProjectCard({ project, index }: { project: typeof projects[0], index: number }) {
  ...
  return (
    <ScrollReveal delay={index * 0.1} className="w-full" width="100%">
      <div className="group block mb-0">
        {/* Carousel Container */}
        <div className="relative overflow-hidden rounded-2xl mb-8 bg-gray-100 border border-gray-200 shadow-sm">
        ...
        >
          <div className="flex-1">
            <h3 className={`text-4xl font-bold mb-3 ${project.link !== "#" ? "hover:underline decoration-2 underline-offset-4" : ""}`}>
              {project.title}
            </h3>
            <p className="text-xl text-gray-500 font-medium mb-4">{project.category}</p>
            <p className="text-gray-600 leading-relaxed text-lg">
              {project.description}
            </p>
          </div>
          {project.link !== "#" && (
            <div className="w-14 h-14 rounded-full border border-gray-200 flex items-center justify-center hover:bg-black hover:text-white hover:border-black transition-all flex-shrink-0 mt-2">
