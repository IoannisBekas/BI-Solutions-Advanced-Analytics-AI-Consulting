                ...
                {activeTab === 'forecast' && (
                    <motion.div key="forecast" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.2 }}>
                        <div className="mb-4 p-4 rounded-xl" style={{ background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.15)' }}>
                            <div className="text-2xl font-bold font-mono metric-value mb-1" style={{ color: '#3B82F6' }}>{report.model_ensemble.ensemble_forecast}</div>
                ...
                {activeTab === 'sentiment' && (
                    <motion.div key="sentiment" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
                        <div className="space-y-3 mb-4">
                            {[
                                { label: 'X Sentiment (Grok API)', score: report.alternative_data.grok_x_sentiment.score, color: '#3B82F6' },
                                { label: 'Reddit Sentiment', score: report.alternative_data.reddit_score, color: '#F97316' },
                                { label: 'News Sentiment', score: report.alternative_data.news_score, color: '#8B5CF6' },
                                { label: 'Composite', score: report.alternative_data.composite_sentiment, color: '#10B981' },
                            ].map(s => (
                ...
                {activeTab === 'altdata' && (
                    <motion.div key="altdata" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {[
                                { label: 'Institutional Flow (13F)', value: report.alternative_data.institutional_flow },
                                { label: 'Insider Activity', value: report.alternative_data.insider_activity },
                                { label: 'Short Interest', value: report.alternative_data.short_interest },
                                { label: 'IV Rank', value: report.alternative_data.iv_rank },
                                { label: 'Options Implied Move', value: report.alternative_data.implied_move },
                                { label: 'Earnings Call NLP', value: report.alternative_data.transcript_score },
                            ].map(item => (
